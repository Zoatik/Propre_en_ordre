...
bool collision(circle c1, square s1, bool init)//init = true : phase d'initialisation
{
    ...
    //std::cout<<fabs(c1.m_center.m_x-s1.m_center.m_x)<<"<?"<<s1.m_size/2+c1.m_radius+int(!init)*epsil_zero<<std::endl;
    if( fabs(c1.m_center.m_x-s1.m_center.m_x)<s1.m_size/2+c1.m_radius+int(!init)*epsil_zero and
        fabs(c1.m_center.m_y-s1.m_center.m_y)<s1.m_size/2+c1.m_radius+int(!init)*epsil_zero)
    {
        //std::cout<<"collision 1"<<std::endl;
        if(fabs(c1.m_center.m_x-s1.m_center.m_x)>s1.m_size/2 and
...
{
    if(distance(c1.m_center, c2.m_center)< (c1.m_radius + c2.m_radius) + int(!init)*epsil_zero)
    {
        return true;
    }
    return false;
...
{
    if(fabs(s1.m_center.m_x-s2.m_center.m_x)< s1.m_size/2 + s2.m_size/2 + int(!init)*epsil_zero and
       fabs(s1.m_center.m_y-s2.m_center.m_y)< s1.m_size/2 + s2.m_size/2 + int(!init)*epsil_zero)
    {
        return true;
    }
    return false;
