...
bool Simulation::generate_particles(File file_infos)
{
    for(unsigned int i(0); i < file_infos.get_particles_infos().size(); i++)
    {
        ...
        {
            if(collision(part.get_shape(),m_particles_vect[j].get_shape()))
            {
...
bool Simulation::generate_robotS(File file_infos)
{
    ...
    if(!check_position(m_robot_S.get_shape()))//check position
    {
        std::cout<<message::spatial_robot_outside(m_robot_S.get_shape().m_center.m_x,
                                                  m_robot_S.get_shape().m_center.m_y);
        return false;
    }
    return true;
}
...
bool Simulation::generate_robotR(File file_infos)
{
    for(unsigned int i(0); i<file_infos.get_robotR_infos().size(); i++)
    {
        ...
        {
            if(collision(curr_robotR.get_shape(),m_robotR_vect[j].get_shape()))
            {
        ...
        {
            if(collision(curr_robotR.get_shape(),m_particles_vect[j].get_shape()))
            {
...
bool Simulation::generate_robotN(File file_infos)
{
    for(unsigned int i(0); i<file_infos.get_robotN_infos().size(); i++)
    {
        ...
        {
            if(collision(curr_robotN.get_shape(),m_robotN_vect[j].get_shape()))
            {
        ...
        {
            if(collision(curr_robotN.get_shape(),m_robotR_vect[j].get_shape()))
            {
        ...
        {
            if(collision(curr_robotN.get_shape(),m_particles_vect[j].get_shape()))
            {
                std::cout<<message::particle_robot_superposition(m_particles_vect[j].get_shape().m_center.m_x,
                                                m_particles_vect[j].get_shape().m_center.m_y,
                                                m_particles_vect[j].get_shape().m_size,
